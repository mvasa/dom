...
int main() 
{
    ...
    {
        int waitstatus;
        waitpid(i, &waitstatus,0);
        
    struct sigaction action;
    action.sa_handler = &handleSignals(signal);
    sigemptyset (&action.sa_mask);
    assert (sigaction (SIGUSR1, &action, NULL) == 0);
    }
}

void handleSignals(char signal){
    Switch(signal){
        case SIGUSR1
            write("successfully handled SIGUSR1 signal");
            break;
        case SIGUSR2
            write("successfully handled SIGUSR2 signal");
            break;
        case SIGINFO
            write("successfully handled SIGINFO signal");
            break;
    }
